...
import { from, interval, mergeMap, Observable } from "rxjs";
...
export class GatewayAnalytics {
    ...
    private calculateDeviceTypeStatistics(events: EventsGroupedByType) {}

    private async measureEvents(): Promise<AnalyticsStatistics> {
        ...
        return {
            ...
            ruuvitag: ruuviTagStatistics,
            miflora: miFloraStatistics,
            deviceMessages: deviceMessageStatistics,
        };
    }

    public observeAnalytics(): Observable<AnalyticsStatistics> {
        return interval(10000).pipe(mergeMap(() => from(this.measureEvents())));
    }
}
